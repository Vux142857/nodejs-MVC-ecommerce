...
const currentModel = containService.modelControl.email;
const mainService = currentModel.emailService; // Service

// Utility
const utilStatusFilter = require("../../utils/utilCreateStatus");
const utilGetParam = require("../../utils/utilParam");
const validateItems = require("../../validates/email");
...
router.post("/send-mail", async function (req, res) {
  ...
  const item = await mainService.getAll({ _id: { $in: req.body.cid } });
  if (Array.isArray(item)) {
    item.forEach(async (item) => {
      await utilMail.sendMail(item.name, message);
    });
  } else {
    await utilMail.sendMail(item.name, message);
  }
  req.flash("successMessage", "Send mail successfully");
  res.redirect(`/admin/${currentModel.index}`);
});
