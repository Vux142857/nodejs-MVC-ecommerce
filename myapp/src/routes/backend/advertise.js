...
const currentModel = containService.modelControl.advertise;
const subModel = containService.modelControl.category;
const mainService = currentModel.advertiseService; // Service
...
const uploadFileMiddleware = utilUpload.upload("thumb", "advertise");
    ...
    try {
      ...
      if (!errorsMsg.isEmpty()) {
        ...
        if (item.id && typeof item.id !== "undefined") {
          await mainService.updateOneById(item.id, advertiseData);
          req.flash("successMessage", "Item updated successfully");
        } else {
          await mainService.create(advertiseData);
